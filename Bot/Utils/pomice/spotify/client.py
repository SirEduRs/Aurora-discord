...
class Client:
    ...
    def __init__(self, client_id: str, client_secret: str) -> None:
        ...
        self._bearer_headers: Dict[Any, Any] = {}
    ...
    async def search(self, *, query: str):
        ...
        spotify_type = result.group("type")  # type: ignore
        spotify_id = result.group("id")  # type: ignore
        ...
        async with self.session.get(
            request_url, headers=self._bearer_headers
        ) as resp:
            if resp.status != 200:
                raise SpotifyRequestException(
        ...
        if spotify_type == "track":
            ...
            tracks = [
                Track(track["track"]) for track in data["tracks"]["items"]
            ...
            while next_page_url is not None:
                ...
                tracks += [
                    Track(track["track"]) for track in next_data["items"]
                    if track["track"] is not None
                ]
