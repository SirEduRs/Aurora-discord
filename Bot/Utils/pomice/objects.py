...
class Track:
    ...
    def __init__(
        ...
        *,
        ...
        self.ctx = ctx
        self.requester = self.ctx.author if ctx else None  # type: ignore
...
class Playlist:
    ...
    def __init__(
        ...
        *,
        ...
        if self.spotify:
            self.tracks: Optional[Any] = tracks
            ...
            self._thumbnail = self.tracks[0].thumbnail
            self._uri = None

        if (index := playlist_info.get("selectedTrack")) == -1:
            self.selected_track = None
        else:
            self.selected_track = self.tracks[index]  # type: ignore
