...
class Node:
    ...
    def __init__(
        ...
        *,
        ...
        self._websocket: aiohttp.ClientWebSocketResponse = None  # type: ignore
    ...
    async def build_track(
        self,
        identifier: str,
        ctx: Optional[commands.Context[AuroraClass]] = None
    ) -> Track:
        """
    ...
    async def get_tracks(
        ...
        *,
        ...
        if not URL_REGEX.match(query) and not re.match(
            r"(?:ytm?|sc)search:.", query
        ):
        ...
        if SPOTIFY_URL_REGEX.match(query):
            ...
            if isinstance(spotify_results, spotify.Track):
                return [
                    Track(
                        ...
                        spotify=True,
                        spotify_track=spotify_results,  # type: ignore
                        info={
            ...
            info: dict = track.get("info")  # type: ignore
...
class NodePool:
    ...
    def get_best_node(
        ...
        if algorithm == NodeAlgorithm.by_ping:
            ...
            nodes = [
                node for node in available_nodes if node._region is voice_region
            ]
            if not nodes:
