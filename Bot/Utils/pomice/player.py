...
class Player(VoiceProtocol):
    ...
    def __init__(
        ...
        *,
        ...
        self._guild = channel.guild if channel else None  # type: ignore

        self._node = node if node else NodePool.get_node()
        self._current: Track = None  # type: ignore
        self._filter: Filter = None  # type: ignore
        self._volume = 100
        self._paused = False
        ...
        self._voice_state: Dict[Any, Any] = {}
    ...
    def position(self) -> float:
        ...
        if self.is_paused:
            return min(self._last_position, current.length)  # type: ignore

        difference = (time.time() * 1000) - self._last_update
        position = self._last_position + difference

        if position > current.length:  # type: ignore
            return 0

        return min(position, current.length)  # type: ignore
    ...
    async def _update_state(self, data: Dict[Any, Any]):
        state: Dict[Any, Any] = data.get("state")  # type: ignore
        self._last_update = time.time() * 1000  # type: ignore
        self._is_connected = state.get("connected")  # type: ignore
        self._last_position = state.get("position")  # type: ignore
    ...
    async def on_voice_state_update(self, data: Dict[Any, Any]):  # type: ignore
    ...
    async def _dispatch_event(self, data: Dict[Any, Any]):
        event_type = data.get("type")
        event: PomiceEvent = getattr(events,
                                     event_type)(data, self)  # type: ignore

        if str(event).startswith(
            "Pomice.TrackEndEvent:"
        ) and event.reason != "REPLACED":
            self._current = None  # type: ignore

        event.dispatch(self._bot)  # type: ignore

        if str(event).startswith("Pomice.TrackStartEvent:"):
    ...
    async def get_tracks(
        ...
        query: str,
        *,
        ctx: Optional[commands.Context['Aurora']] = None,
        ...
        return await self._node.get_tracks(
            query, ctx=ctx, search_type=search_type
        )
    ...
    async def seek(self, position: float) -> float:
        ...
        if position < 0 or position > self._current.original.length:  # type: ignore
    ...
    async def set_volume(self, volume: int) -> int:
        ...
        await self._node.send(
            op="volume", guildId=str(self.guild.id), volume=volume
        )
        self._volume = volume
        return self._volume
    ...
    async def reset_filter(self):
        ...
        await self._node.send(
            op="filters", guildId=str(self.guild.id), **_payload
        )
        await self.seek(self.position)
        self._filter = None
