...
class Equalizer(Filter):
    ...
    """
    def __init__(self, *, levels: List[Any]) -> None:
        super().__init__()

        self.eq = self._factory(levels)
        self.raw = levels

        self.payload: Dict[str, Any] = {"equalizer": self.eq}

    def _factory(self, levels: List[Any]) -> List[Any]:
        _dict: Dict[Any, Any] = collections.defaultdict(int)
